...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api/push/vapid-public-key` | Lets the client bootstrap without bundling the key at build time. |
| `POST` | `/api/push/subscribe` | ── POST /push/subscribe ────────────────────────────────────────────────── |
| `DELETE` | `/api/push/subscribe` | Removes ALL push subscriptions for this device. Used when the user |
...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api/users/search` | Query: q=<string> (min 2 chars), limit=<int> (1..25, default 10) |
| `GET` | `/api/users/:device_id` | Public-shape lookup — returns 404 if user doesn't exist OR has no |
